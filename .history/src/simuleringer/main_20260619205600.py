...
class Body:
    ...
    def __init__(self, x, y, xp=0, yp=0, batch=None, radius=BODY_RADIUS, color=WHITE, program=None, add_trail=True):
        ...
        self.shape = pyg.shapes.Circle(*nc2sc(self.x, self.y), radius=radius, color=color, batch=self.batch)

        verts = [-1, -1, 1, -1, 1, 1, -1, 1]
        self.vert_list = self.program.vertex_list(4, pyg.gl.GL_TRIANGLE_FAN, position=("f", verts))

        if self.add_trail:
            self.trail = [nc2sc(self.x, self.y)] * TRAIL_LENGTH

            self.line = pyg.shapes.MultiLine(*self.trail, color=self.color, batch=self.batch)
    ...
    def update(self):
        ...
        if self.add_trail:
            ...
            self.line.delete()
            self.line = pyg.shapes.MultiLine(*self.trail, thickness=4, color=self.color, batch=self.batch)
...
class ThreeBodyProblemSimulation(pyg.window.Window):
    def __init__(self, *args, **kwargs):
        ...
        # General Setup
        self.set_location((SCREEN_WIDTH - WIDTH) // 2, (SCREEN_HEIGHT - HEIGHT) // 2)
        ...
        self.program = pyg.graphics.shader.ShaderProgram(
            pyg.graphics.shader.Shader(vertex_source, "vertex"), pyg.graphics.shader.Shader(fragment_source, "fragment")
        )

        # Simulation Variables
        vars = [-0.5, 0, 0.5, 0, 0, 0]

        # Bodies
        self.body_1 = Body(vars[0], vars[1], batch=self.batch, program=self.program, color=(220, 220, 160))

        self.body_2 = Body(vars[2], vars[3], batch=self.batch, program=self.program, color=(100, 220, 100))

        self.body_3 = Body(
            vars[4], vars[5], batch=self.batch, program=self.program, color=(180, 255, 180), add_trail=False
        )
    ...
    def simulation_step(self, dt):
        ...
        _, y_values = solve_2nd_order_ivp_interact(
            threebodyfunc,
            [*self.body_1.vars, *self.body_2.vars, *self.body_3.vars],
            [
                *self.body_1.vars_p,
                *self.body_2.vars_p,
                *self.body_3.vars_p,
...
if __name__ == "__main__":
    ...
    """
    sim = ThreeBodyProblemSimulation(width=WIDTH, height=HEIGHT, caption="3 Body Problem Simulation")
