...
solver_steps = {"euler": euler_step, "mod_euler": improved_euler_step, "rk4": rk4_step}
...
def solve_ivp_interact(
    ...
    while t_curr < t_max:
        ...
        t_curr, y_curr = solver(func, y_curr, t_curr, dt_intern)
...
def solve_2nd_order_ivp_interact(
    ...
    if isinstance(y_curr, Iterable) or isinstance(yp_curr, Iterable):
        dim = len(y_curr)
        reduced_func = lambda t, *vars: np.array([*vars[dim:], *func(t, *vars)])
    else:
        reduced_func = lambda t, y, yp: np.array([yp, func(t, y, yp)])

    reduced_y_curr = np.array([y_curr, yp_curr]).flatten()

    return solve_ivp_interact(reduced_func, reduced_y_curr, t, method, dt, dt_intern)
